...
class DataPreprocessor:
    ...
    def _calculate_2d_channel_coordinates(self) -> np.ndarray:
        ...
        positions_3d = np.asarray([montage.get_positions()['ch_pos'][ch_name] for ch_name in g.ch_names])
    ...
    def _calculate_image_coordinates(self) -> Tuple[np.ndarray, np.ndarray]:
        """
        Calculates image coordinates for the (64x64) image.
    ...
    def _calculate_signal_power(self, session_signals: np.ndarray) -> np.ndarray:
        ...
        :param session_signals: Signal data from a session, shape (num_valid_trial, num_channels, num_motor_imagery_samples_per_run)
    ...
    @staticmethod
    def _calculate_frequency_feature(window_data: np.ndarray) -> np.ndarray:
        """
        Calculates the power across the three frequency bands for one channel as described in the Fadel paper
        :param window_data: data of one window, numpy array of shape (num_motor_imagery_samples_per_run/num_time_windows, )
        ...
        freq_hz = np.fft.fftfreq(n=window_data.shape[-1], d=1 / g.sampling_frequency_hz)
        for idx, frequency_band_hz in enumerate(g.frequency_bands_hz):
    ...
    def _interpolate_all_data(self, frequency_features: np.ndarray) -> np.ndarray:
        ...
        :return: interpolated image for each trial, numpy ndarray of shape (num_valid_trial, num_windows, 64, 64, 3)
        """
